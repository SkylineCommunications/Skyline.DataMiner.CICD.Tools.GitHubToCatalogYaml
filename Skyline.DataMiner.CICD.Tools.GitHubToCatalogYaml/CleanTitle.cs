namespace Skyline.DataMiner.CICD.Tools.GitHubToCatalogYaml
{
	...
	public class CleanTitle
	{
		...
		public CleanTitle(string githubRepoName)
		{
			...
			var ownerSeperatorIndex = githubRepoName.IndexOf('/');
			if (ownerSeperatorIndex != -1)
			{
				githubRepoName = githubRepoName.Substring(ownerSeperatorIndex + 1);
			}
			...
			if (splitDash.Length > 2 && splitDash[0].Length < 10)
			{
				...
				if (Constants.ArtifactTypeMap.TryGetValue(splitDash[1].ToUpper(), out foundItemType))
				{
		...
		/// </summary>
		public string FoundItemType { get; set; }
